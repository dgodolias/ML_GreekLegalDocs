...
def format_mean_report_to_string(mean_report_dict, report_class_labels_str, include_ci=False):
    ...
    if include_ci:
        # Adjusted header for new interval display
            ...
            if include_ci and 'precision' in metrics and isinstance(metrics['precision'], dict):
                for metric_name in ['precision', 'recall', 'f1-score']:
                    if metric_name in metrics:
                        ...
                        moe_val = metrics[metric_name]['moe']
                        ...
                        report_str += f"{'N/A':>32}" # Placeholder if metric is missing
            else: 
...
def load_and_preprocess_data(dataset_config, use_subset, subset_percentage, random_state=42):
    ...
    if not dataset_split:
        ...
        if len(ds.keys()) > 0:
            split_to_use = list(ds.keys())[0]
            print(f"Attempting to use: '{split_to_use}'")
    ...
    if 'text' not in dataset_split.column_names or 'label' not in dataset_split.column_names:
        print(f"Error: 'text' or 'label' column not found. Available: {dataset_split.column_names}")
        return None, None, None, None
    ...
    if use_subset:
        ...
        if len(texts_to_process) == 0:
            print("Error: Subset empty.")
            return None, None, None, None
    
    if not isinstance(labels_to_process, np.ndarray):
        labels_to_process = np.array(labels_to_process)
...
def perform_k_fold_cv_and_report(
    ...
    n_splits_cv, 
    vectorizer_class, vectorizer_params, 
    feature_name, cv_output_dir, random_state=42
):
    """Performs K-Fold CV, generates and saves reports."""
    print(f"\n--- Performing {n_splits_cv}-Fold Cross-Validation with {feature_name} ---")
    
    if len(texts_to_process) < n_splits_cv:
        print(f"Error: Samples ({len(texts_to_process)}) < N_SPLITS_CV ({n_splits_cv}). Cannot perform K-Fold CV.")
        return False
    ...
    if degrees_freedom > 0:
        ...
        print("Warning: Cannot calculate t_critical with N_SPLITS_CV <= 1. CI for metrics will not be calculated.")
    ...
    for fold_idx, (train_indices, val_indices) in enumerate(skf.split(texts_to_process_np, labels_to_process)):
        ...
        fold_vectorizer = vectorizer_class(**vectorizer_params)
        X_train_fold_transformed = fold_vectorizer.fit_transform(X_train_fold_texts)
        X_val_fold_transformed = fold_vectorizer.transform(X_val_fold_texts)
        
        fold_svm = SVC(kernel='linear', random_state=random_state)
    ...
    # Adjusted print message as JSON is no longer saved
    print(f"Mean classification report (TXT) with CIs saved to {mean_report_str_path}") 
    
    return True

def run_svm_classification(X_train_features, X_test_features, y_train, y_test, 
                           feature_type="BoW", labels=None, target_names=None, output_dir=None, random_state=42):
    """Trains SVM, predicts, evaluates, and optionally saves reports."""
    ...
    if output_dir:
        ...
        safe_feature_type = "".join(c if c.isalnum() else "_" for c in feature_type)

        report_txt_path = os.path.join(output_dir, f"classification_report_{safe_feature_type}.txt")
        with open(report_txt_path, "w", encoding='utf-8') as f:
            f.write(f"Classification Report for {feature_type}\n")
            f.write(f"Accuracy: {accuracy:.4f}\n\n")
            f.write(report_str if report_str else "Report could not be generated.")
        print(f"Single run classification report (TXT) saved to {report_txt_path}")

        if report_dict: 
            report_json_path = os.path.join(output_dir, f"classification_report_{safe_feature_type}.json")
