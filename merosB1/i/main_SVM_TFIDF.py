...
def main_tfidf(): # Changed
    ...
    vectorizer_class = TfidfVectorizer # Changed
    ...
    if PERFORM_K_FOLD_CV:
        run_output_dir = os.path.join(output_base_dir, f"tfidf_{DATASET_CONFIG}_cv_reports") # Changed
    else:
        run_output_dir = os.path.join(output_base_dir, f"tfidf_{DATASET_CONFIG}_single_run_reports") # Changed
    ...
    if PERFORM_K_FOLD_CV:
        ...
        X_train_texts, X_test_texts, y_train, y_test = sk_train_test_split(
            texts_proc, labels_proc, test_size=0.2, random_state=RANDOM_STATE, # Using 0.2 test_size as in original
