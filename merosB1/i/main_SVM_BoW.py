import sys
import os
import numpy as np # For array conversion for KFold
import json # For saving report dictionaries
import time
from scipy.stats import t # For confidence interval calculation
...
def format_mean_report_to_string(mean_report_dict, report_class_labels_str, include_ci=False):
    ...
    else: # Fallback for old format if CI data not present, or for simpler display
        header = f"{'':<20}{'precision':>10}{'recall':>10}{'f1-score':>10}{'support':>10}\n\n"
    
    report_str = header

    for label in report_class_labels_str: # Iterate only over class labels first
        if label in mean_report_dict:
            ...
            else: # Fallback to simple display if CI data structure isn't as expected
    ...
    report_str += "\n"
    # Handle overall averages like 'macro avg', 'weighted avg'
...
def main_bow():
    ...
    cv_output_dir = os.path.join(output_base_dir, f"bow_{DATASET_CONFIG}_cv_reports")
    
    if PERFORM_K_FOLD_CV:
        os.makedirs(cv_output_dir, exist_ok=True)
        print(f"INFO: Reports will be saved in '{cv_output_dir}'")
    else:
        # Adjust single run output dir as well if desired, or keep it simpler
        single_run_output_dir = os.path.join(output_base_dir, f"bow_{DATASET_CONFIG}_single_run_reports")
        os.makedirs(single_run_output_dir, exist_ok=True) 
        print(f"INFO: Single run reports will be saved in '{single_run_output_dir}'")


    # ... (rest of the initial print statements and data loading logic remains the same) ...
    ...
    if PERFORM_K_FOLD_CV:
        ...
        for fold_idx, (train_indices, val_indices) in enumerate(skf.split(texts_to_process_np, labels_to_process)):
            ...
            fold_vectorizer = CountVectorizer(max_features=5000)
        ...
        # Pass only class labels to the formatter for the class-specific part
